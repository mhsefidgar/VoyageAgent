...
export async function GET(request: Request) {
  ...
  try {
    ...
    const hotels = await fetch(`${baseUrl}/v1/reference-data/locations/hotels/by-city?cityCode=${encodeURIComponent(cityCode)}`, { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' });
    ...
    const offers = await fetch(`${baseUrl}/v3/shopping/hotel-offers?hotelIds=${encodeURIComponent(ids)}&checkInDate=${encodeURIComponent(checkInDate)}&checkOutDate=${encodeURIComponent(checkOutDate)}&adults=${encodeURIComponent(adults)}`, { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' });
